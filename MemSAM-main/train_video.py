...
def main():
    ...
    parser.add_argument('--task', default='CAMUS', help='task or dataset name: CAMUS_Video_Full or EchoNet_Video')
    ...
    train_dataset = EchoDataset(opt.data_path, opt.train_split, tf_train, img_size=args.encoder_input_size)
    ...
    val_dataset = EchoDataset(opt.data_path, opt.val_split, tf_val, img_size=args.encoder_input_size)
    # trainloader = DataLoader(train_dataset, batch_size=opt.batch_size, shuffle=True, num_workers=8, pin_memory=True)
    trainloader = DataLoader(train_dataset, batch_size= 4, shuffle=True, num_workers=8, pin_memory=True)
